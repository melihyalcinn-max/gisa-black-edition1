...
async function unlock(){ const pass=$('#adminPass')?.value||''; const h=await sha256(pass); if(h===ADMIN_HASH){ sessionStorage.setItem('gisa_admin_ok','1'); $('#loginGate').style.display='none'; $('#cmsApp').style.display='grid'; status('Admin açıldı. İçerik değişiklikleri merkezi sistem’a kaydedilince yayın altyapısı otomatik deploy eder.'); loadLocal(); } else { $('#loginMsg').textContent='Şifre hatalı.'; } }
...
async function gh(method,path,body){ const c=cfg(); if(!c.owner||!c.repo||!c.token) throw new Error('Owner, repo ve merkezi sistem token zorunlu'); const url=`https://api.github.com/repos/${c.owner}/${c.repo}/contents/${path}${method==='GET'?`?ref=${c.branch}`:''}`; const r=await fetch(url,{method:method==='GET'?'GET':'PUT',headers:{Authorization:`Bearer ${c.token}`,Accept:'application/vnd.github+json'},body:method==='GET'?undefined:içerik verisi.stringify(body)}); const j=await r.json(); if(!r.ok) throw new Error(j.message||'merkezi sistem hata'); return j; }
...
async function loadSelected(){ status('Yükleniyor...'); const j=await gh('GET',selected); const txt=decodeB64(j.content); $('#editor').value=içerik verisi.stringify(içerik verisi.parse(txt),null,2); $('#editor').dataset.sha=j.sha; status('Yüklendi: '+selected); }
async function saveSelected(){ içerik verisi.parse($('#editor').value); status('merkezi sistem’a kaydediliyor...'); const content=encodeB64($('#editor').value); await gh('PUT',selected,{message:'GISA CMS update '+selected,content,sha:$('#editor').dataset.sha,branch:cfg().branch}); status('Kaydedildi. merkezi sistem değişti; yayın altyapısı otomatik yeni versiyonu yayına alır.'); }
async function uploadImage(e){ const f=e.target.files[0]; if(!f)return; status('Görsel merkezi sistem’a yükleniyor...'); const reader=new FileReader(); reader.onload=async()=>{ const base64=reader.result.split(',')[1]; const safe=f.name.toLowerCase().replace(/[^a-z0-9.]+/g,'-'); const filePath='public/uploads/'+Date.now()+'-'+safe; await gh('PUT',filePath,{message:'GISA CMS upload '+safe,content:base64,branch:cfg().branch}); const publicPath='/uploads/'+filePath.split('/').pop(); $('#imagePath').value=publicPath; status('Görsel yüklendi. içerik verisi içinde bu yolu kullan: '+publicPath); }; reader.readAsDataURL(f); }
window.addEventListener('DOMContentLoaded',()=>{
  ...
  $('#newFragrance')?.addEventListener('click',()=>{ selected='content/fragrances.json'; $('#editor').value=içerik verisi.stringify({fragrances:[{slug:'yeni-koku',name:'Yeni Koku Profili',family:'Premium',tr:'Kısa Türkçe açıklama',description:'Detaylı açıklama',mood:'Lüks, temiz ve kalıcı atmosfer',intensity:'Orta',longevity:'Yüksek',top:['bergamot'],heart:['musk'],base:['amber'],sectors:['kozmetik']} ]},null,2); status('Yeni koku şablonu hazır. Mevcut dosyayı yükleyip içine eklemek daha güvenlidir.'); });
  $('#newPost')?.addEventListener('click',()=>{ selected='content/blog.json'; $('#editor').value=içerik verisi.stringify({posts:[{slug:'yeni-yazi',title:'Yeni İçerik Başlığı',category:'Guide',excerpt:'Kısa özet',body:['Paragraf 1','Paragraf 2']}]},null,2); status('Yeni blog şablonu hazır.'); });
});
})();

(function(){
  const $=(s,r=document)=>r.querySelector(s); const $$=(s,r=document)=>Array.from(r.querySelectorAll(s));
  function preview(){const ed=$('#editor'), box=$('#livePreview'); if(!ed||!box)return; try{const data=içerik verisi.parse(ed.value||'{}'); const keys=Object.keys(data); let html='<h4>içerik verisi geçerli ✅</h4><p>Üst alanlar: '+keys.join(', ')+'</p>'; if(data.site) html+=`<p><b>Site:</b> ${data.site.name||'Gisa'} / ${data.site.description||''}</p>`; if(data.fragrances) html+=`<p><b>Koku sayısı:</b> ${data.fragrances.length}</p>`; if(data.sectors) html+=`<p><b>Sektör sayısı:</b> ${data.sectors.length}</p>`; if(data.posts) html+=`<p><b>Blog sayısı:</b> ${data.posts.length}</p>`; html+='<pre>'+içerik verisi.stringify(data,null,2).slice(0,1200)+'</pre>'; box.innerHTML=html; box.style.borderColor='rgba(200,164,106,.22)';}catch(e){box.innerHTML='<b>içerik verisi hatalı ❌</b><p>'+e.message+'</p>'; box.style.borderColor='rgba(255,70,70,.55)';}}
  window.addEventListener('DOMContentLoaded',()=>{const ed=$('#editor'); ed?.addEventListener('input',preview); setInterval(preview,2000);
    ...
    $('#writeModuleOrder')?.addEventListener('click',()=>{const ed=$('#editor'); if(!ed)return; let data={}; try{data=içerik verisi.parse(ed.value||'{}')}catch(e){alert('Önce geçerli site.json yükle');return;} if(!data.site)data.site={}; data.site.homeModules=$$('.drag-item').map(x=>x.dataset.module); ed.value=içerik verisi.stringify(data,null,2); preview();});
  });
})();
